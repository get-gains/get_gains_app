...
|---------|---------|------------|--------|
| `elevated` | Standard container | `card` | 1px `border` |
| `outlined` | Border emphasis, no fill | `transparent` | 1px `border` |
| `flat` | Minimal, blends with bg | `transparent`/subtle | none |
| `gradient` | Feature/promotional | Gradient | none |
...
|------|---------|---------------|
| `sm` | `spacing3` (12px) | `radiusMd` (12px) |
| `md` | `spacing5` (20px) | `radiusLg` (16px) |
| `lg` | `spacing6` (24px) | `radiusXl` (20px) |
...
| Hover | Background shifts to `surface2` |
...
- `elevation > 0` - Subtle shadow based on value
...
class _AppCardState extends State<AppCard> {
  ...
  EdgeInsetsGeometry get _padding {
    ...
    return switch (widget.size) {
      AppCardSize.sm => const EdgeInsets.all(12),
      AppCardSize.md => const EdgeInsets.all(20),
      AppCardSize.lg => const EdgeInsets.all(24),
    };
  }
  ...
  BorderRadiusGeometry get _borderRadius {
    ...
    return switch (widget.size) {
      AppCardSize.sm => BorderRadius.circular(AppTheme.radiusMd),
      AppCardSize.md => BorderRadius.circular(AppTheme.radiusLg),
      AppCardSize.lg => BorderRadius.circular(AppTheme.radiusXl),
    };
  }
  ...
  BoxDecoration _getDecoration(bool isDark) {
    return switch (widget.variant) {
      AppCardVariant.elevated => BoxDecoration(
        ...
        border: Border.all(
          color: widget.borderColor ?? (isDark ? AppColors.borderDark : AppColors.borderLight),
        ),
      ...
      AppCardVariant.outlined => BoxDecoration(
        color: _isHovered && _isInteractive
            ? (isDark ? AppColors.surface1Dark : AppColors.surface1Light).withOpacity(0.5)
            : Colors.transparent,
        borderRadius: _borderRadius,
        border: Border.all(
          color: widget.borderColor ?? (isDark ? AppColors.borderDark : AppColors.borderLight),
      ...
      AppCardVariant.flat => BoxDecoration(
        color: _getBackgroundColor(isDark).withOpacity(_isHovered && _isInteractive ? 0.8 : 0.5),
        borderRadius: _borderRadius,
      ),
...
Horizontal card optimized for list displays.
...
Card for displaying metrics with optional trends.
          ...
          if (trend != null) ...[
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: (trendPositive ? AppColors.success : AppColors.error).withOpacity(0.2),
...
// Size variants
AppCard(size: AppCardSize.sm, child: CompactContent())
AppCard(size: AppCardSize.md, child: StandardContent())
AppCard(size: AppCardSize.lg, child: LargeContent())
```
...
- ✅ Interactive cards: Announced as buttons to screen readers
- ✅ Minimum touch target: 44x44px for tappable cards
- ✅ Focus states: Visible ring for keyboard navigation
- ✅ Color contrast: Sufficient for text on all card backgrounds
