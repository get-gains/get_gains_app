...
/// Screen for creating a new custom exercise.
///
/// Available to all authenticated users (not coach-only).
/// Returns the created [ExerciseModel] via `context.pop(exercise)` when
/// successfully submitted, so callers can react to the result.
///
/// Does **not** include pose-form ("form reference") management — that stays
/// coach-only via `POST /pose/forms`.
...
class _CreateExerciseScreenState extends ConsumerState<CreateExerciseScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Listen for state changes — pop with the created exercise on success.
    ref.listen<CreateExerciseState>(createExerciseProvider, (previous, next) {
      if (next is CreateExerciseSuccess) {
        ...
        // Refresh the global exercise list so it appears in pickers.
        ref.read(exerciseListProvider.notifier).refresh();
        // Return the created exercise to the caller.
        context.pop(next.exercise);
