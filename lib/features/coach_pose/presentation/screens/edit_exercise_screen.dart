...
class _EditExerciseScreenState extends ConsumerState<EditExerciseScreen> {
  ...
  void initState() {
    ...
    _equipment = List<String>.from(widget.exercise.equipmentNeeded);
  }
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(editExerciseProvider);
    final isDark = Theme.of(context).brightness == Brightness.dark;

    // Listen for state changes
    ref.listen<EditExerciseState>(editExerciseProvider, (previous, next) {
      if (next is EditExerciseSuccess) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Exercise "${next.exercise.name}" updated!'),
        ...
        // Refresh the exercise details
        ref.read(exerciseDetailProvider(widget.exercise.id).notifier).setExercise(next.exercise);
        
        context.pop();
      } else if (next is EditExerciseError) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
    ...
    final isLoading = state is EditExerciseLoading;
  ...
  void _onSubmit() {
    ...
    ref
        .read(editExerciseProvider.notifier)
        .updateExercise(
