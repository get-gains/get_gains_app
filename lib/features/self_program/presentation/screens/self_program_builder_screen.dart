...
import 'self_add_routine_sheet.dart';
import 'self_add_exercise_sheet.dart';
import 'self_edit_exercise_sheet.dart';
import 'self_edit_routine_sheet.dart';
...
class SelfProgramBuilderScreen extends ConsumerStatefulWidget {
  const SelfProgramBuilderScreen({
    super.key,
  ...
  @override
  ConsumerState<SelfProgramBuilderScreen> createState() =>
      _SelfProgramBuilderScreenState();
}

class _SelfProgramBuilderScreenState extends ConsumerState<SelfProgramBuilderScreen> {
  ...
  Future<void> _init() async {
    final notifier = ref.read(selfProgramBuilderProvider.notifier);
  ...
  Widget build(BuildContext context) {
    ...
    if (!_hasInitialised && state is SelfProgramBuilderLoaded) {
  ...
  Widget _buildBody(SelfProgramBuilderState state, bool isDark) {
    return switch (state) {
      SelfProgramBuilderInitial() => _buildMetaStep(null, isDark),
      SelfProgramBuilderLoading() => const Center(
        child: CircularProgressIndicator(),
      ),
      SelfProgramBuilderError(:final error) => Center(
        child: Column(
      ...
      SelfProgramBuilderLoaded(:final program, :final currentStep) => _buildStepper(
        program,
        currentStep,
        isDark,
      ),
      ...
      children: [
        Expanded(
          child: sortedRoutines.isEmpty
              ? AppEmptyState.compact(
                  ...
                  onAction: () => _showSelfAddRoutineSheet(program),
                )
              : ReorderableListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return _RoutineCard(
                      ...
                      routine: routine,
                      isDark: isDark,
                      onEdit: () => _showSelfEditRoutineSheet(program, routine),
        ...
        _BottomActionBar(
          ...
          center: AppButton.outline(
            ...
            icon: Icons.add,
            isFullWidth: true,
            onPressed: () => _showSelfAddRoutineSheet(program),
            textStyle: const TextStyle(
              fontSize: 18,
              fontWeight: FontWeight.bold,
            ),
  ...
  Future<void> _showSelfAddRoutineSheet(ClientProgramModel program) async {
    final added = await showSelfAddRoutineSheet(
      context: context,
  ...
  Future<void> _showSelfEditRoutineSheet(
    ClientProgramModel program,
    ProgramRoutineModel routine,
  ) async {
    await showSelfEditRoutineSheet(
      context: context,
      programId: program.id,
      routine: routine,
    );
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return _ExpandableRoutineExerciseCard(
                      ...
                      isDark: isDark,
                      onAddExercise: () =>
                          _showSelfAddExerciseSheet(program, routine),
                      onEditExercise: (ex) =>
                          _showSelfEditExerciseSheet(program, routine, ex),
                      onDeleteExercise: (ex) =>
  ...
  Future<void> _showSelfAddExerciseSheet(
    ClientProgramModel program,
    ProgramRoutineModel routine,
  ) async {
    await showSelfAddExerciseSheet(
      context: context,
      programId: program.id,
      aprId: routine.id,
  ...
  Future<void> _showSelfEditExerciseSheet(
    ClientProgramModel program,
    ProgramRoutineModel routine,
    ProgramRoutineExerciseModel exercise,
  ) async {
    await showSelfEditExerciseSheet(
      context: context,
      programId: program.id,
      aprId: routine.id,
      exercise: exercise,
    ...
    return Column(
      children: [
        ...
        _BottomActionBar(
          ...
          trailing: AppButton(
            label: program.isActive ? 'Save Changes' : 'Start Program',
