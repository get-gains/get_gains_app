...
class _AssignRoutineSheetContentState
  ...
  void initState() {
    ...
    final routinesState = ref.read(coachRoutinesProvider);
    if (routinesState is CoachRoutinesInitial) {
      Future.microtask(
  ...
  Widget _buildRoutineDropdown(
    CoachRoutinesState state,
    bool isDark,
    ThemeData theme,
  ) {
    if (state is CoachRoutinesLoading || state is CoachRoutinesInitial) {
      return const Center(child: CircularProgressIndicator());
    }

    final routines = state is CoachRoutinesLoaded
        ? state.routines
        : <RoutineSummaryModel>[];
